...
- `ExtractionService` принимает `repository.Querier` и `workerPythonClient`; `Initiate(ctx, docID, orgID, questions)` создаёт задачу `resolve_keys` и триггерит Python (best-effort). `WorkerService.HandleStatusUpdate` при `resolve_keys completed` вызывает `triggerExtract` (upsert ключей → **tenant-check mdDoc.OrganizationID == doc.OrganizationID** → создание `extract`-задачи), при `extract completed` — `handleExtractCompleted` (**batch** upsert `extracted_data` одним `BatchUpsertExtractedData` вызовом: параллельные массивы `key_ids`/`extracted_values` через `unnest`).
